...
test('exports', function (assert) {
  ...
  assert.equal(typeof uuid.v1, 'function');
  assert.equal(typeof uuid.parse, 'function');
  assert.equal(typeof uuid.unparse, 'function');
});

test('parse/unparse', function (assert) {
  var id = '00112233445566778899aabbccddeeff';
  assert.equal(
    unparse(parse(id.substr(0, 10))),
    '00112233-4400-0000-0000-000000000000',
    'Short parse'
  );
  assert.equal(
    unparse(parse('(this is the uuid -> ' + id + id)),
...
test('clock regression by nsec', function (assert) {
  ...
  assert.equal(
    parseInt(uidtnb.split('-')[3], 16) - parseInt(uidtn.split('-')[3], 16),
    1,
